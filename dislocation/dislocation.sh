...
if [ "$(hostname)" = "amd01" ]; then
    ...
    cd $CURRENT_DIRECTORY
    export CUDA_VISIBLE_DEVICES=$GPU_CORE
    nohup mpirun -np 1 -cpu-set $GPU_CORE ~/Downloads/lammps-stable_2Aug2023_update3/src/lmp_kokkos_cuda_v100 -k on g 1 -sf kk -pk kokkos newton on neigh half -in in.relax_HEA.$ALLOY.$SESSION_NAME > STDOUT &
    echo "-----------------------------------------"
    echo "Info of this session: "
    ps aux | grep in.relax_HEA.$ALLOY.$SESSION_NAME
    echo "-----------------------------------------"
