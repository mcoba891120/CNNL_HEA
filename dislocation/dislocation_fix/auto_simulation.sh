...
if [ "$SIMULATION_MODE" = "relax" ]; then
    ...
    # STRUCTURE_PATH="compression/NiCoTiZrHf_111/structure/${STRUCTURE_NAME}_mc.lmp"
    echo $STRUCTURE_PATH
...
if [ "$(hostname)" = "amd01" ]; then
    ...
    nohup mpirun -np 1 -cpu-set 0 ~/Downloads/lammps-stable_2Aug2023_update3/src/lmp_kokkos_cuda_v100 -k on g 1 -sf kk -pk kokkos newton on neigh half -in in.$SIMULATION_MODE.$ALLOY.$SESSION_NAME > STDOUT &
    echo "-----------------------------------------"
    echo "Info of this session: "
    ps aux | grep in.$SIMULATION_MODE.$ALLOY.$SESSION_NAME
    echo "-----------------------------------------"
