...
def discover_folders(base_path):
    """
    Automatically discover change_ratio folders and their subfolders (next and from...to...).
    ...
    # Find all change_ratio folders
    change_ratio_folders = []
    for item in os.listdir(base_path):
        item_path = os.path.join(base_path, item)
        if os.path.isdir(item_path) and "change_ratio" in item:
            change_ratio_folders.append(item_path)
            print(f"Found change_ratio folder: {item}")
    
    if not change_ratio_folders:
        warnings.warn("No change_ratio folders found in the current directory!")
        return folder_paths
    
    # Process each change_ratio folder
    for change_ratio_folder in change_ratio_folders:
        # Add the main change_ratio folder
        folder_paths.append(change_ratio_folder)
        
        # Look for subfolders (next and from...to...)
        try:
            subfolders = os.listdir(change_ratio_folder)
            
            # Find next subfolders
            next_folders = [f for f in subfolders if os.path.isdir(os.path.join(change_ratio_folder, f)) and "next" in f.lower()]
            for next_folder in next_folders:
                next_path = os.path.join(change_ratio_folder, next_folder)
                folder_paths.append(next_path)
                print(f"  Found next subfolder: {os.path.basename(change_ratio_folder)}/{next_folder}")
            
            # Find from...to... subfolders
            from_to_folders = [f for f in subfolders if os.path.isdir(os.path.join(change_ratio_folder, f)) and "from" in f.lower() and "to" in f.lower()]
            for from_to_folder in from_to_folders:
                from_to_path = os.path.join(change_ratio_folder, from_to_folder)
                folder_paths.append(from_to_path)
                print(f"  Found from...to... subfolder: {os.path.basename(change_ratio_folder)}/{from_to_folder}")
                
                # Look for next subfolders within from...to... folders
                try:
                    from_to_subfolders = os.listdir(from_to_path)
                    from_to_next_folders = [f for f in from_to_subfolders if os.path.isdir(os.path.join(from_to_path, f)) and "next" in f.lower()]
                    for from_to_next_folder in from_to_next_folders:
                        from_to_next_path = os.path.join(from_to_path, from_to_next_folder)
                        folder_paths.append(from_to_next_path)
                        print(f"    Found next subfolder in from...to...: {os.path.basename(change_ratio_folder)}/{from_to_folder}/{from_to_next_folder}")
                except PermissionError:
                    warnings.warn(f"Permission denied when accessing {from_to_path}")
                except Exception as e:
                    warnings.warn(f"Error accessing {from_to_path}: {e}")
                
        except PermissionError:
            warnings.warn(f"Permission denied when accessing {change_ratio_folder}")
        except Exception as e:
            warnings.warn(f"Error accessing {change_ratio_folder}: {e}")
...
def process_folder(folder_path, max_screen=8, base_path=None, length_factor=1.0):
    ...
    # Create hierarchical name for subfolders
...
def plot_individual_folders(data_list, output_folder):
    ...
    for data in data_list:
        ...
        if data['barrier_info']:
            ...
            plt.annotate(f'Energy Barrier: {barrier["energy_barrier"]:.3f}', 
            ...
            plt.annotate(f'Forward: {barrier["forward_barrier"]:.3f}', 
                        xy=(barrier['max_screen'], barrier['max_energy']),
                        xytext=(10, -20), textcoords='offset points',
