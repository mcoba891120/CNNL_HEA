...
for i in range(natoms):
    ####################
    dispx = pos2[i][0]-pos1[i][0]
    if dispx > 0.5*lenx: dispx -= lenx
    if dispx < -0.5*lenx: dispx += lenx
    dispy = pos2[i][1]-pos1[i][1]
    if dispy > 0.5*leny: dispy -= leny
    if dispy < -0.5*leny: dispy += leny
    dispz = pos2[i][2]-pos1[i][2]
...
ase.io.write("HEA_init_screw3.data",atoms2,format="lammps-data",specorder=["Ni","Co","Ti","Zr","Hf"],masses=True)
