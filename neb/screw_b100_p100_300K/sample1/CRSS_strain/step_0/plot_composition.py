...
def main():
    # 讀取檔案：優先 *.data
    files = sorted(Path(".").glob("configureD_numBCC_sb100p100_*.data"), key=_extract_suffix_int)
    if not files:
        files = sorted(Path(".").glob("configureD_numBCC_sb100p100_*"), key=_extract_suffix_int)
    if not files:
        raise FileNotFoundError("找不到 configureD_numBCC_sb100p100_* 檔案。請確認腳本與檔案在同資料夾。")
