...
def monte_carlo_simulation(num_iterations, mc_temp, lammps_data_file, reference_file, gpu_id=None):
    ...
    for iter in range(1, num_iterations + 1):
        ...
        e_new = ovito_strain_sum("current_state.data", reference_file)

        # Update the strain history (only keep last 10)
        if len(last_strains) >= 10:
            last_strains.pop(0)
        last_strains.append(e_new)
        ...
        if rank == 0:
            with open("MC_record.txt", "a") as f:
                f.write(f"{iter} {e_new} {strain_min} {reason} type_{type1}-{type2}\n")
...
if __name__ == "__main__":
    ...
    monte_carlo_simulation(num_iterations=500000, mc_temp=300, gpu_id=gpu_id, 
                           lammps_data_file="strain_stress/mc300k/after_relax.data",
                           reference_file="strain_stress/mc300k/atomic_strain_ref.data")
