...
df_cleaned_atoms = pd.DataFrame(atom_data_cleaned, columns=['ID', 'Type', 'x', 'y', 'z', 'c1', 'c2', 'c3'])
df_cleaned_atoms[['ID', 'Type']] = df_cleaned_atoms[['ID', 'Type']].astype(int)

# Define the ranges for the analysis
first_range_cleaned = df_cleaned_atoms[(df_cleaned_atoms['ID'] >= 1) & (df_cleaned_atoms['ID'] <= 34560)]
second_range_cleaned = df_cleaned_atoms[(df_cleaned_atoms['ID'] > 34560) & (df_cleaned_atoms['ID'] <= 69120)]
...
element_map_cleaned = {1: 'Co', 2: 'Ni', 3: 'Ti', 4: 'Zr', 5: 'Hf'}
