...
if [ "$SIMULATION_MODE" = "relax" ]; then
    ...
    # STRUCTURE_PATH="monte_carlo/var${VAR_NUM}_${TOTAL_ATOM}_900k/mc_folder/emin.data"
    # STRUCTURE_PATH="relaxation/$ALLOY_ORIENTATION/var${VAR_NUM}_${TOTAL_ATOM}_900k/after_relax.data"
    # STRUCTURE_PATH="relaxation/NiCoTiZrHf_110/var3_9000_300k/MC_stress1/mc_folder/stress_min.data"
...
if [ "$(hostname)" = "amd01" ]; then
    ...
    nohup mpirun -np 1 -cpu-set $NUM_GPU ~/Downloads/lammps-stable_2Aug2023_update3/src/lmp_kokkos_cuda_v100 -k on g 1 -sf kk -pk kokkos newton on neigh half -in in.${SIMULATION_MODE}z.$ALLOY.$SESSION_NAME > STDOUT &
    echo "-----------------------------------------"
    echo "Info of this session: "
    ...
    read -p "Enter the gpu number: " NUM_GPU
    export CUDA_VISIBLE_DEVICES=$NUM_GPU
    nohup mpirun -np 1 -cpu-set $NUM_GPU /home/cnnltmp01/Downloads/lammps-stable_2Aug2023_update3/src/lmp_kokkos_cuda_p100 -k on g 1 -sf kk -pk kokkos newton on neigh half -in in.${SIMULATION_MODE}z.$ALLOY.$SESSION_NAME > STDOUT &
    echo "-----------------------------------------"
    echo "Info of this session: "
    ps aux | grep in.$SIMULATION_MODE.$ALLOY.$SESSION_NAME
    echo "-----------------------------------------"
